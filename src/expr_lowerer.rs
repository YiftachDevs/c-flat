...
pub enum IRExprResult<'ctx> {
    ...
    FunctionName(IRScopeId, String, Option<IRExprValueResult<'ctx>>, Option<IRImplId>),
...
impl<'ctx> CodeLowerer<'ctx> {
    ...
    pub fn lower_impl_fun_name(&mut self, type_id: IRTypeId, fun_name: &str, opt_self_value: Option<IRExprValueResult<'ctx>>, span: Span) -> Result<Option<IRExprResult<'ctx>>, CompilerError> {
        if let Some(impl_id) = self.find_impl_of_fun(type_id, fun_name, Some(span))? {
            ...
            if !fun_def.templates.templates.is_empty() {
                return Ok(Some(IRExprResult::FunctionName(ir_impl.scope, fun_name.to_string(), opt_self_value,  Some(impl_id))));
            } else {
                ...
                return Ok(Some(IRExprResult::Function(fun, opt_self_value)));
            }
        }
        Ok(None)
    }
