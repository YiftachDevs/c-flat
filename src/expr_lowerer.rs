...
impl<'ctx> CodeLowerer<'ctx> {
   ...
   /*  pub fn get_type_from_expr(&self, ctx_scope: IRScopeId, expr: &ExprNode) -> Result<IRTypeId, CompilerError> {
    ...
    /*pub fn lower_expr_value(&mut self, ir_fun_scope: &mut IRFunContext<'ctx>, expr: &ExprNode, context_type: Option<IRTypeId>) -> Result<IRExprValueResult<'ctx>, CompilerError> {
    ...
    pub fn get_type(&mut self, ir_context: &mut IRContext<'ctx>, expr: &ExprNode, context_type: Option<IRTypeId>) -> Result<IRTypeId, CompilerError> {
        ...
        return Err(self.error("Expression is not a type", None, Some(expr.span)));
    }
    ...
    pub fn lower_expr(&mut self, ir_context: &mut IRContext<'ctx>, expr: &ExprNode, context_type: Option<IRTypeId>) -> Result<IRExprResult<'ctx>, CompilerError> {
        match &expr.value {
            ...
            ExprNodeEnum::Scope(scope) => {
                Ok(IRExprResult::Value(self.lower_scope(ir_context, scope, context_type)?))
            },
            ExprNodeEnum::Name(name, _) => {
                self.lower_expr_first_name(ir_context, name, expr.span)
            },
            ExprNodeEnum::PostfixOpr(opr, left_expr, right_expr) => {
            ...
            ExprNodeEnum::InfixOpr(opr, left_expr, right_expr) => {
                ...
                Ok(IRExprResult::CommaSeperated(Box::new(self.lower_expr(ir_context, left_expr, context_type)?), right_expr.clone()))
            }
            _ => todo!("lower_expr 2")
        }
    }

    /*fn lower_exprs(&mut self, ir_fun_scope: &mut IRFunContext<'ctx>, expr: &ExprNode, context_type: Option<IRTypeId>) -> Result<IRExprResult<'ctx>, CompilerError> {
    ...
    fn lower_exprs_values(&mut self, ir_fun_scope: &mut IRFunContext<'ctx>, expr: &ExprNode, context_type: Option<IRTypeId>) -> Result<Vec<IRExprValueResult<'ctx>>, CompilerError> {
    ...
    fn lower_postfix_opr_templates(&mut self, ir_context: &mut IRContext<'ctx>, left_expr: &Box<ExprNode>, right_expr: &Option<Box<ExprNode>>, context_type: Option<IRTypeId>) -> Result<IRExprResult<'ctx>, CompilerError> {
        ...
        let templates = if let IRExprResult::FunctionName(parent_scope, fun_name) = left_expr_result {
            let fun_def = self.find_fun_def_in_scope(parent_scope, fun_name.as_str()).unwrap().1;
            fun_def.templates.unwrap()
    ...
    fn lower_args_values(&mut self, ir_context: &mut IRContext<'ctx>, args_expr: &Option<Box<ExprNode>>, context_types: &Vec<IRTypeId>) -> Result<Vec<BasicValueEnum<'ctx>>, CompilerError> {
        ...
        if let Some(expr) = args_expr {
            ...
            while i < args_len {
                let expr_result = self.lower_expr(ir_context, cur_expr.as_ref(), Some(context_types[i]))?;
                i += 1;
    ...
    fn lower_expr_first_name(&mut self, ir_context: &mut IRContext<'ctx>, name: &str, span: Span) -> Result<IRExprResult<'ctx>, CompilerError> {
        ...
        let parent_scope = self.get_context_parent_scope(ir_context);
        if let Some((actual_scope, struct_def)) = self.find_struct_def_in_scope(parent_scope, name) {
            return Ok(self.lower_struct_name(actual_scope, struct_def, span)?);
        }
        if let Some(module) = self.get_module_scope_in_scope(parent_scope, name) {
